  ...
  <article v-else class="max-w-4xl mx-auto py-8 px-2 sm:px-0">
    ...
    <header class="mb-12">
      <div class="flex flex-wrap items-center gap-2 mb-4 justify-center">
        ...
        <span v-else-if="post.article_language === 'english'" class="bg-gray-800 px-3 py-1 rounded-full text-sm flex items-center gap-1">
          🇬🇧 English
        </span>
    ...
    <footer class="border-t border-gray-700 pt-8 mt-16">
      <div class="flex flex-col sm:flex-row justify-between items-center gap-6">
        <NuxtLink to="/blog" class="text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1">
        ...
        <div class="flex flex-col items-center sm:items-end gap-2">
          ...
          <div class="flex gap-2">
            <button @click="shareArticle('twitter')" class="p-2 rounded-full bg-gray-800 hover:bg-gray-700 transition-colors" title="Share on Twitter">
              <Icon name="mdi:twitter" class="w-5 h-5" />
            </button>
            <button @click="shareArticle('facebook')" class="p-2 rounded-full bg-gray-800 hover:bg-gray-700 transition-colors" title="Share on Facebook">
              <Icon name="mdi:facebook" class="w-5 h-5" />
            </button>
            <button @click="shareArticle('linkedin')" class="p-2 rounded-full bg-gray-800 hover:bg-gray-700 transition-colors" title="Share on LinkedIn">
              <Icon name="mdi:linkedin" class="w-5 h-5" />
            </button>
            <button @click="shareArticle('link')" class="p-2 rounded-full bg-gray-800 hover:bg-gray-700 transition-colors" title="Copy Link">
        ...
        <div class="grid sm:grid-cols-2 gap-6">
          ...
          >
            ...
            <p class="text-gray-400 text-sm line-clamp-2">
              {{ relatedPost.description || (relatedPost.body && relatedPost.body.value ? relatedPost.body.value[0][2].substring(0, 120) + '...' : '') }}
            </p>
          </NuxtLink>
        </div>
      </div>
